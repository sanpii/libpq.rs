...
pub fn socket_poll(
    ...
    let status =
        unsafe { pq_sys::PQsocketPoll(sock, for_read as i32, for_write as i32, end_time.into()) };

    if status > 0 {
        Ok(())
    } else if status == 0 {
        Err(crate::errors::Error::Timeout)
    } else {
...
"#
    ...
    fn close_portal() {
        ...
        let results = conn.exec("declare curs1 refcursor");
        assert_eq!(results.status(), crate::Status::CommandOk);
        let results = conn.exec("open curs1 for select 1");
        assert_eq!(results.status(), crate::Status::CommandOk);
    ...
    fn send_close_portal() {
        ...
        let results = conn.exec("declare curs2 refcursor");
        assert_eq!(results.status(), crate::Status::CommandOk);
        let results = conn.exec("open curs2 for select 1");
        assert_eq!(results.status(), crate::Status::CommandOk);
